...
let () =
  (*register "z" (fun () -> Player.(move_player (player1()) Cst.paddle_v_up));
  register "s" (fun () -> Player.(move_player (player1()) Cst.paddle_v_down));*)
  register "d" (fun () -> Player.(move_player (player1()) Cst.paddle_v_right));
  register "q" (fun () -> Player.(move_player (player1()) Cst.paddle_v_left));
